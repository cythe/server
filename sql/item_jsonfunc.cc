  ...
  while (a <= a_end)
  {
    if (b > b_end)
    {
             ...
             a->n_item == 0)
      {
        a=
         (json_path_step_t*)
           mem_root_dynamic_array_get_val((MEM_ROOT_DYNAMIC_ARRAY *)a_arr,
                                          (a-(json_path_step_t*)(a_arr->buffer))+1);
        if (a > a_end)
          return 0;
      }
      return -2;
...
step_failed:
    ...
    b=
      (json_path_step_t*)
            mem_root_dynamic_array_get_val((MEM_ROOT_DYNAMIC_ARRAY *)b_arr,
                                            (b-(json_path_step_t*)(b_arr->buffer))+1);
    continue;
                      ...
                      MEM_ROOT_DYNAMIC_ARRAY *array_size)
{
  ...
  return json_path_parts_compare(&(a->steps), temp_a + 1, a->last_step,
                                 &(b->steps), temp_b + 1, b->last_step,
                                 vt, array_size);
}
...
bool Item_func_json_valid::fix_length_and_dec(THD *thd)
{
  ...
  mem_root_dynamic_array_init(&current_mem_root, PSI_NOT_INSTRUMENTED,
                              ...
                              JSON_DEPTH_DEFAULT, JSON_DEPTH_DEFAULT, MYF(0));
...
bool Item_func_json_value::fix_length_and_dec(THD *thd)
{
  ...
  mem_root_dynamic_array_init(&current_mem_root, PSI_NOT_INSTRUMENTED,
                              &json_depth_array, sizeof(int), NULL, JSON_DEPTH_DEFAULT,
                              JSON_DEPTH_DEFAULT, MYF(0));
                                          ...
                                          char **out_val, int *value_len)
{
  ...
  *type= possible_multiple_values ? JSON_VALUE_ARRAY : JSON_VALUE_NULL;
  ...
  {
    int tmp= 0, index= (int)(p.last_step-(json_path_step_t*)(p.steps.buffer));
    if (has_negative_path && je.value_type == JSON_VALUE_ARRAY &&
        (json_skip_array_and_count(&je, &tmp) ||
         mem_root_dynamic_array_set_val(&json_depth_array, &tmp, index)))
      goto error;
